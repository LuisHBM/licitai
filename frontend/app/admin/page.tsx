...
export default function PainelAdminPage() {
  ...
  const apiStatus = 'operacional' as const;
  ...
  const handleStartColeta = () => {
    ...
    const tick = () => {
      if (step >= total) {
        ...
        setColetas((prev) =>
          prev.map((c) => (c.id === novaColeta.id ? { ...c, status: 'concluida', registros } : c))
        );
      ...
      <div className="max-w-7xl mx-auto px-8 py-8">
        ...
        <section id="coletas" className="bg-white border border-[#E2E8F0] rounded-lg p-6 mb-6">
          ...
          <div className="grid grid-cols-4 gap-4 mb-5">
            <div>
              <label className="block text-[13px] text-[#111827] font-medium mb-1.5">
                Modalidade
              </label>
              <select
                value={modalidade}
            ...
            <table className="w-full text-[13px]">
              <thead>
                <tr className="bg-[#F5F7FA] border-b border-[#E2E8F0]">
                  {['Data/Hora', 'Modalidade', 'UF', 'Período', 'Registros', 'Status', 'Ação'].map(
                    (col) => (
                      <th key={col} className="px-5 py-3 text-left text-[#6B7280] font-medium">
                        {col}
                      </th>
                    )
                  )}
