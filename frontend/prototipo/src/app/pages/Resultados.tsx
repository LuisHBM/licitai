...
export function Resultados() {
  ...
  const handleSearch = () => {
    ...
    setCurrentPage(1);
    setSearchParams({ q: inputValue.trim(), mode: isSemanticActive ? 'semantica' : 'textual' });
  };
  ...
  const filtered = ALL_LICITACOES.filter((l) => {
    ...
    if (situacaoFilters.length > 0 && !situacaoFilters.map((s) => s.toLowerCase()).includes(l.situacao)) return false;
      ...
      <div className="bg-[#1A3A5C] px-8 py-4">
        <div className="max-w-7xl mx-auto flex gap-3 items-center">
          ...
          <div className="flex items-center gap-2 ml-4">
            <button
              onClick={() => setIsSemanticActive(!isSemanticActive)}
      ...
      <div className="max-w-7xl mx-auto px-8 py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <p className="text-[15px] text-[#111827]">
              <span className="font-medium text-[#1A3A5C]">{filtered.length} licitações</span> encontradas
              {query && (
        ...
        {hasActiveFilters && (
          <div className="flex flex-wrap items-center gap-2 mb-5">
            ...
            {valorMin && (
              <FilterChip label={`Mín: R$ ${Number(valorMin).toLocaleString('pt-BR')}`} onRemove={() => { setValorMin(''); setCurrentPage(1); }} />
            )}
            {valorMax && (
              <FilterChip label={`Máx: R$ ${Number(valorMax).toLocaleString('pt-BR')}`} onRemove={() => { setValorMax(''); setCurrentPage(1); }} />
            )}
            <button
                  ...
                  <div className="space-y-2">
                    {MODALIDADES.map((m) => (
                      <label key={m} className="flex items-center gap-2 cursor-pointer group">
                        ...
                        />
                        <span className="text-[13px] text-[#111827] group-hover:text-[#1A3A5C] transition-colors">{m}</span>
                      </label>
                    ))}
                  </div>
                </div>
                    ...
                    {SITUACOES.map((s) => (
                      <label key={s} className="flex items-center gap-2 cursor-pointer group">
                        <input
                          ...
                          className="w-4 h-4 rounded border-[#E2E8F0] text-[#1A3A5C] focus:ring-[#2E6DA4]"
                        />
                        <span className="text-[13px] text-[#111827] group-hover:text-[#1A3A5C] transition-colors">{s}</span>
                      </label>
                    ))}
                  </div>
                </div>
              ...
              <>
                <div className="space-y-3">
                  {pageItems.map((lic) => (
                    <ResultCard
                      key={lic.id}
                      lic={lic}
                      showSimilaridade={isSemanticActive}
                ...
                <div className="flex items-center justify-between mt-8">
                  <p className="text-[13px] text-[#6B7280]">
                    Exibindo {(currentPage - 1) * PER_PAGE + 1}–{Math.min(currentPage * PER_PAGE, filtered.length)} de {filtered.length}
                  </p>
                  <div className="flex items-center gap-1">
                    ...
                    {pageNumbers.map((p, i) =>
                      p === '...' ? (
                        <span key={`ellipsis-${i}`} className="w-9 h-9 flex items-center justify-center text-[13px] text-[#6B7280]">
                          …
                        </span>
                      ) : (
                        <button
...
interface ResultCardProps {
  lic: ReturnType<typeof buildMockData>[number];
  showSimilaridade: boolean;
  formatValor: (v: number) => string;
}

function ResultCard({ lic, showSimilaridade, formatValor }: ResultCardProps) {
  const situacaoColor = {
  ...
  return (
    <div className="bg-white border border-[#E2E8F0] rounded-lg p-5 hover:border-[#2E6DA4] hover:shadow-sm transition-all group">
      <div className="flex items-start justify-between gap-4 mb-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className={`px-2.5 py-0.5 text-[12px] font-medium rounded border capitalize ${situacaoColor}`}>
            {lic.situacao}
          </span>
        </div>
        {showSimilaridade && (
          <div className="flex items-center gap-1.5 flex-shrink-0">
            ...
            <span className="text-[12px] font-medium text-[#2E6DA4]">{lic.similaridade}%</span>
          </div>
        )}
      </div>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[17px] text-[#1A3A5C] font-medium">{formatValor(lic.valorEstimado)}</p>
        </div>
        <Link
          to={`/licitacao/${lic.id}`}
